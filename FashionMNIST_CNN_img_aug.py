...
# Example: assuming images are 224px wide
image_width = 28
max_translate_px = 10
translate_fraction = max_translate_px / image_width  # ~0.0446

# Equivalent transform
augmenter = v2.Compose([
    v2.RandomAffine(
        degrees=0,  # No rotation
        translate=(translate_fraction, 0),  # X-axis only, fraction of width
        fill=0  # Equivalent to mode='constant' with black fill
    )
])
...
class FMNIST_Dataset(Dataset):
    ...
    def collate_func(self, batch):
        ...
        if self.aug:
            imgs = self.aug(imgs)
        imgs = torch.tensor(imgs)[:,None,:,:].to(device)/255.0
        classes = torch.tensor(classes).to(device)
        return imgs, classes


def get_data():
    training_dataset = FMNIST_Dataset(train_images, train_targets, aug=augmenter)
    training_dataloader = DataLoader(training_dataset, batch_size=16, 
            shuffle=True, collate_fn=training_dataset.collate_func)
